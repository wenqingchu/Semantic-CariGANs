...
net = SiameseNetwork()
net.load_state_dict(torch.load('models/siamese.pth.tar'))
net.cuda()

net.eval()
cari_names = []
cari_f = open('cari_annos/annos.txt')
cari_enc= np.zeros((len(cari_f.readlines()), 128))
cari_f = open('cari_annos/annos.txt')
for i, cari_name in enumerate(cari_f.readlines()):
    cari_name = cari_name.strip()
    img0 = Image.open('cari_annos/label/' + cari_name)
...
photo_name = 'Scarlett_Johansson_P00001.png'
img0 = Image.open(photo_name)
img0 = channel_1toN(img0, 10)
img0 = img0.cuda()
